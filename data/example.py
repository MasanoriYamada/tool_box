...
def check_seq_data(data_loader):
    train_iter = data_loader.get_train_loader()
    train_data_size = 0
    train_label_set = set()
    for data, label, idx in train_iter:
        train_data_size += len(data)
        if label.numpy().shape[1:] == ():
            train_label_set = train_label_set.union(label.numpy())
        else:
            pass
        print(
            'size:{} {}, type:{} {} min{}, max{}'.format(data.size(), label.size(), type(data), type(label), data.min(),
                                                         data.max()))
    
    test_iter = data_loader.get_test_loader()
    test_data_size = 0
    test_label_set = set()
    for data, label, idx in test_iter:
        test_data_size += len(data)
        if label.numpy().shape[1:] == ():
            test_label_set = test_label_set.union(label.numpy())
        else:
